...
def bootstrap_diff(a, b, statistic=np.mean, n_boot=2000, alpha=0.05, method="bca", seed=0) -> dict:
...
def main():
    ...
    print(
        f"  {'mean diff':<14} {'t-test':<10} {t['point']:>10.3f} {t['se']:>9.3f} "
        f"[{t['ci_low']:>8.3f}, {t['ci_high']:>8.3f}]"
    )
    print(
        f"  {'mean diff':<14} {'boot-BCa':<10} {bm['point']:>10.3f} {bm['se']:>9.3f} "
        f"[{bm['ci_low']:>8.3f}, {bm['ci_high']:>8.3f}]"
    )

    bd_med = bootstrap_diff(a, b, statistic=np.median, n_boot=2000, seed=2)
    bd_q90 = bootstrap_diff(a, b, statistic=lambda x: np.quantile(x, 0.9), n_boot=2000, seed=3)
    print(
        f"  {'median diff':<14} {'boot-BCa':<10} {bd_med['point']:>10.3f} {bd_med['se']:>9.3f} "
        f"[{bd_med['ci_low']:>8.3f}, {bd_med['ci_high']:>8.3f}]"
    )
    print(
        f"  {'p90 diff':<14} {'boot-BCa':<10} {bd_q90['point']:>10.3f} {bd_q90['se']:>9.3f} "
