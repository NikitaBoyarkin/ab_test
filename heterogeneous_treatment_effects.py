...
def fit_het(df, ref_segment="desktop"):
    ...
    df["segment"] = pd.Categorical(df["segment"],
                                   categories=[ref_segment] + [s for s in df["segment"].unique()
                                                               if s != ref_segment])
    ...
    for g in df["segment"].cat.categories:
        ...
        rows.append({"segment": g, "ate": diff, "se": se,
...
def main():
    ...
    for g, r in ate.iterrows():
        ...
        print(f"  {str(g):<10} {r['ate']:>8.3f} {r['se']:>7.3f} "
    ...
    for term in inter:
        ...
        print(f"  {term:<28} coef={model.params[term]:+.3f}  "
              f"p={p:.4f}{star}")
