...
def simulate_daily(n_days=21, n_per_day=400, kind="novelty", seed=0):
    ...
    rows = []
    for d, eff in zip(t, effect, strict=True):
        for treat in (0, 1):
            n = n_per_day // 2
...
def diagnose(result, alpha=0.05):
    ...
    return (
        "novelty (effect decays over time)"
        if result["inter_coef"] < 0
        else "primacy (effect grows over time)"
    )
...
def main():
    ...
    for kind in ["novelty", "primacy", "stable", "none"]:
        ...
        for _, r in traj.iterrows():
            bar = "+" * int(max(0, r["uplift"] * 10)) + "-" * int(max(0, -r["uplift"] * 10))
            print(
                f"    day {int(r['day']):>2}  {r['uplift']:+.3f} "
